...
void  Picture:: write_picture(string file_name, vector<double**>photo, int height1, int width1)
{
    int width = width1;
    int height = height1;
   ...
   // {
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                pixels[i][j].redComponent = 255 * photo[0][i][j];
                pixels[i][j].greenComponent = 255 * photo[1][i][j];
                pixels[i][j].blueComponent = 255 * photo[2][i][j];
               // if (photo[0][i][j] != 0)
                //{
